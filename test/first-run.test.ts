...
describe("console first-run setup screen (#251)", () => {
  test("isSetupComplete requires model and gateway token (solo default, no bot step)", () => {
    expect(isSetupComplete({ hasModel: true, hasGatewayToken: true })).toBe(true);
  ...
  test("shouldShowFirstRun: shown when incomplete and not skipped", () => {
    ...
    expect(shouldShowFirstRun({ hasModel: true, hasGatewayToken: true }, false)).toBe(false); // complete
  });

  test("firstRunView computes progress and per-step done flags", () => {
    const v = firstRunView({ hasModel: true, hasGatewayToken: true });
