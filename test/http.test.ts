...
test("console serves the sidebar-bots module (#276)", async () => {
  const base = start({
    consoleDir: join(import.meta.dir, "..", "src", "gateway", "console"),
  });
  const res = await fetch(`${base}/console/sidebar-bots.js`);
  expect(res.status).toBe(200);
  expect(res.headers.get("content-type")).toContain("application/javascript");
  const body = await res.text();
...
test("console serves the chat-history module (#284)", async () => {
  const base = start({
    consoleDir: join(import.meta.dir, "..", "src", "gateway", "console"),
  });
  const res = await fetch(`${base}/console/chat-history.js`);
  expect(res.status).toBe(200);
  expect(res.headers.get("content-type")).toContain("application/javascript");
  const body = await res.text();
  expect(body).toContain("sessionMessages");
...
test("console serves the scope-defaults module (#277)", async () => {
  const base = start({
    consoleDir: join(import.meta.dir, "..", "src", "gateway", "console"),
  });
  const res = await fetch(`${base}/console/scope-defaults.js`);
  expect(res.status).toBe(200);
  expect(res.headers.get("content-type")).toContain("application/javascript");
  const body = await res.text();
  expect(body).toContain("resolvePanelScope");
...
test("console serves the approval-card module (#295)", async () => {
  const base = start({
    consoleDir: join(import.meta.dir, "..", "src", "gateway", "console"),
  });
  const res = await fetch(`${base}/console/approval-card.js`);
  expect(res.status).toBe(200);
  expect(res.headers.get("content-type")).toContain("application/javascript");
  const body = await res.text();
  expect(body).toContain("approvalCardTitle");
